...
def test_projector_rebuilds_legacy_schema_and_recovers_historical_invoice_payments(
    tmp_path: Path,
) -> None:
    event_store = EventStore(
        database_url=f"sqlite:///{(tmp_path / 'events.db').as_posix()}",
    ...
    with legacy_engine.begin() as connection:
        ...
        connection.execute(
            text(
                """
                INSERT INTO projection_meta (singleton_id, schema_version)
                VALUES (1, 3)
                """
            )
        )
    ...
    assert applied == 4
    assert projector.get_last_applied_event_id() == 4
    assert projector.list_invoice_payments(invoice_id="card-1:2026-04") == [
        {
...
def test_projector_recovers_legacy_reimbursement_cancellation_date(
    tmp_path: Path,
) -> None:
    event_store = EventStore(
        database_url=f"sqlite:///{(tmp_path / 'events.db').as_posix()}",
    ...
    assert projector.list_reimbursements()[0]["canceled_at"] == "2026-03-05T09:30:00Z"


def test_projector_reopens_canceled_reimbursement(
    tmp_path: Path,
) -> None:
    event_store = EventStore(
        database_url=f"sqlite:///{(tmp_path / 'events.db').as_posix()}",
    ...
    reimbursement = projector.list_reimbursements()[0]
    assert reimbursement["status"] == "pending"
    assert reimbursement.get("canceled_at") is None
...
def test_projector_moves_invoice_payment_balance_when_payment_account_changes(
    tmp_path: Path,
) -> None:
    event_store = EventStore(
        database_url=f"sqlite:///{(tmp_path / 'events.db').as_posix()}",
    ...
    assert applied == 6
    assert projector.get_invoice_payment("payment-1") == {
        "payment_id": "payment-1",
        "invoice_id": "card-1:2026-04",
        "card_id": "card-1",
        "account_id": "acc-2",
    ...
    assert projector.list_transactions(account_id="acc-2") == [
        {
            "transaction_id": "payment-1:invoice-payment",
            "occurred_at": "2026-03-20T12:00:00Z",
            "type": "expense",
...
def test_projector_keeps_paid_invoice_consistent_after_non_financial_card_purchase_update(
    tmp_path: Path,
) -> None:
    event_store = EventStore(
        database_url=f"sqlite:///{(tmp_path / 'events.db').as_posix()}",
    ...
    march_invoice = next(
        invoice
        for invoice in projector.list_invoices(card_id="card-1")
...
def test_projector_materializes_investment_movements_and_monthly_overview(
    ...
    assert movements == [
        ...
        },
        {
            ...
            "contribution_amount": 30_00,
            "dividend_amount": 0,
            "reinvested_dividend_amount": 5_00,
            "cash_amount": 30_00,
            "invested_amount": 35_00,
            "cash_delta": -30_00,
            "invested_delta": 35_00,
    ...
    assert overview == {
        ...
        "totals": {
            ...
            "dividend_total": 0,
            "reinvested_dividend_total": 5_00,
            "external_contribution_total": 30_00,
            "dividend_received_total": 0,
            "dividend_reinvested_total": 5_00,
